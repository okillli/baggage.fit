...
export function CompareTable({
  ...
  if (sortedAirlines.length === 0) {
    let message = 'No airlines found with data for this bag type.';
    if (searchQuery) {
      message = `No airlines matching "${searchQuery}".`;
    } else if (airlines.length === 0) {
      message = 'No airlines match your current filters.';
    }
    return (
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      {/* Desktop Table */}
      <div className="hidden md:block overflow-x-auto rounded-xl border border-foreground/10 bg-white">
        <table className="w-full">
          <caption className="sr-only">
            Airline {bagType} baggage size and weight limits
          </caption>
              ...
              return (
                ...
                >
                  <td className="py-4 px-4">
                    <button
                      type="button"
                      aria-label={`View details for ${airline.name}`}
                      className={cn(
                        'flex items-center gap-3 text-left',
                        onAirlineClick && 'cursor-pointer hover:text-accent-on-light transition-colors'
                      )}
                      onClick={() => onAirlineClick?.(airline.code)}
                    >
                      {isBest && (
                        <span className="flex items-center gap-1 px-2 py-0.5 bg-accent text-foreground text-xs font-bold rounded">
                          <Crown className="w-3 h-3" />
                          BEST
                        </span>
                      )}
                      <div>
                        <p className="font-medium text-foreground">{airline.name}</p>
                        <p className="text-xs text-foreground/50 font-mono">{airline.code}</p>
                      </div>
                    </button>
                  </td>
                  <td className="py-4 px-4 font-mono text-foreground/80">
                  ...
                  <td className="py-4 px-4 text-sm text-foreground/60 max-w-xs truncate">
                    {allowance?.notes || '—'}
                  </td>
                  <td className="py-4 px-4">
                    <a
                      ...
                      className="inline-flex items-center gap-1 text-sm text-accent-on-light hover:underline"
                    >
        ...
        {sortedAirlines.map(({ airline, allowance }, index) => {
          ...
          return (
            <div
              key={airline.code}
              className={cn(
                'bg-white border border-foreground/10 rounded-lg p-4',
            ...
            >
              <button
                ...
                aria-label={`View details for ${airline.name}`}
                className={cn(
                  'flex items-center justify-between mb-3 w-full text-left',
              ...
              <div className="grid grid-cols-2 gap-3 text-sm mb-3">
                <div>
                  <p className="text-foreground/50 text-xs mb-1">Max Size</p>
                  <p className="font-mono text-foreground/80">{formatDimensions(allowance?.maxCm || null, unit)}</p>
                </div>
                <div>
                  <p className="text-foreground/50 text-xs mb-1">Weight</p>
                  <p className={cn('text-foreground/80', getWeightCellColor(userWeightKg, allowance?.maxKg))}>
                    {allowance?.maxKg ? formatWeight(allowance.maxKg, weightUnit) : '—'}
                  </p>
                </div>
              </div>
              {allowance?.notes && (
                <p className="text-xs text-foreground/60 mb-3">{allowance.notes}</p>
              )}
              <a
                href={airline.links.policy}
                target="_blank"
...
function getWeightCellColor(
  ...
  if (userWeightKg == null || !maxKg) return 'text-foreground/80';
  return userWeightKg <= maxKg ? 'text-accent-on-light font-medium' : 'text-red-500 font-medium';
}
